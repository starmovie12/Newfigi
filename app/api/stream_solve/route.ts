...
// =============================================================================
// MISSION 3 — TASK C: External HubCloud API endpoint has been DELETED.
// The old API_MAP.hubcloud entry is removed. HubCloud is now solved natively
// via solveHubCloudNative() from lib/solvers.ts.
// =============================================================================
const API_MAP = {
  ...
  // hubcloud: DELETED — replaced by native solveHubCloudNative()
    ...
    async start(controller) {
      ...
      const processLink = async (linkData: any, idx: number) => {
        ...
        try {
          ...
          // =================================================================
          // MISSION 3 — HUBCLOUD (FINAL) — NOW USING NATIVE SOLVER
          // Replaced: fetchWithUA(API_MAP.hubcloud + ...) 
          // With:     await solveHubCloudNative(currentLink)
          // Also saves all_available_buttons + best_button_name to Firestore.
          // =================================================================
          if (currentLink.includes('hubcloud') || currentLink.includes('hubcdn')) {
            ...
            try {
              ...
              // ✅ FIX: Set result and return HERE so we don't fall through to "Unrecognized"
              send({ id: lid, status: 'error', msg: e.message });
              finalResults.set(lid, { ...linkData, status: 'error', error: e.message, logs });
              return;
            }
          }

          // --- FINAL FALLBACK: Only triggers for genuinely unrecognized link formats ---
      ...
      if (taskId) {
        try {
          ...
          if (taskDoc.exists) {
            ...
            // Update existing links by matching both index (lid) AND URL for precision
            const updatedLinks = existingLinks.map((existingLink: any, existingIdx: number) => {
              // First try: match by the link's original index (most reliable)
              let matchingResult = finalResults.get(existingIdx);

              // Fallback: if no index match, try matching by URL + taskId scope
              if (!matchingResult) {
                for (const [_key, value] of finalResults.entries()) {
                  if (value.link === existingLink.link && value.name === existingLink.name) {
